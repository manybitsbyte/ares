...
struct Backend : ares::Platform {
  ...
  auto input(ares::Node::Input::Input input) -> void override {
    ...
    auto player = port && port->name() == "Controller Port 2" ? 1 : 0;
    ...
    if(input->name() == "1") bit = One;
    if(input->name() == "2") bit = Two;
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_alloc(u32 size) -> void* {
  return std::malloc(size);
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_free(void* memory) -> void {
  std::free(memory);
}
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_load(const u8* data, u32 size) -> int {
  ...
  if(auto port = backend.root->find<ares::Node::Port>("Cartridge Slot")) {
    ...
    return fail("The Master System core did not expose a cartridge slot");
  }
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_video_width() -> u32 {
  return backend.videoWidth;
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_video_height() -> u32 {
  return backend.videoHeight;
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_audio_data() -> const float* {
  return backend.audioSamples.data();
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_audio_frames() -> u32 {
...
//this returns void for the same reason ares_ms_run_frame does: the export unwinds and JS is handed
//the unwind's value rather than the function's. the size is read back with ares_ms_state_size,
//following the ares_ms_audio_frames / ares_ms_audio_data split, which exists for the same reason.
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_state_save(int synchronize) -> void {
  backend.stateBytes.clear();
  if(!backend.root) { stateFail("No cartridge is loaded"); return; }
  auto s = ares::MasterSystem::system.serialize(synchronize != 0);
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_state_size() -> u32 {
  return backend.stateBytes.size();
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_state_data() -> const u8* {
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_state_load(const u8* data, u32 size) -> int {
  if(!backend.root) return stateFail("No cartridge is loaded");
  if(!data || !size) return stateFail("State is empty");
  serializer s{data, size};
  if(!ares::MasterSystem::system.unserialize(s)) return stateFail("Not a valid state for the Master System core");
  return 1;
}
...
//0, which is the answer, not a failure. no scheduler is entered, so unlike ares_ms_state_save this
//could have returned the size — it splits size out anyway to read the same way.
EMSCRIPTEN_KEEPALIVE auto ares_ms_save_ram_save() -> void {
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_save_ram_size() -> u32 {
  return backend.saveRamBytes.size();
}

EMSCRIPTEN_KEEPALIVE auto ares_ms_save_ram_data() -> const u8* {
...
//after ares_ms_load and before running a frame, and the machine is left where booting with the
//battery already in it would have left it.
EMSCRIPTEN_KEEPALIVE auto ares_ms_save_ram_load(const u8* data, u32 size) -> int {
  if(!backend.root) return stateFail("No cartridge is loaded");
...
EMSCRIPTEN_KEEPALIVE auto ares_ms_switch_count() -> u32 {
  return (u32)co_switch_count;
}
#endif

EMSCRIPTEN_KEEPALIVE auto ares_ms_error() -> const char* {
